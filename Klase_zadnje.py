...
class PrikazIgre(object):
    ...
    def prikazi_odluku_o_nastavku():
        unos = input("Želite li ponovno igrati (da/ne)? ")
        return unos
...
class Igra(object):
    ...
    def igranje_milijunasa(self):
        ...
        if self.provjera_odluke_o_nastavku():
            main()
    ...
    def provjera_odluke_o_nastavku(self):
        odluka = None
        while odluka is None:
            odgovor = self.prikaz.prikazi_odluku_o_nastavku()
            if odgovor.upper() == "NE":
                odluka = False
            elif odgovor.upper() == "DA":
                odluka = True
        return odluka
