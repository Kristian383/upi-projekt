...
class Jocker(object):
    ...
    def pitaj_publiku(lista_odgovora):
        ...
        rezultat = []
        zamjena = lista_odgovora[:]  # kopirana lista odgovora
...
class Igra(object):
        ...
        for p in self.pitanja.pitanja_15:
            ...
            while True:
                ...
                if odluka == "1":
                    ...
                    # igrac odgovara
                    self.igrac_odgovara(trenutno_pitanje, i)
                    break
    ...
    def igrac_odgovara(self, pitanje_objekt, redni_broj):
        odgovor = self.prikaz.igrac_odgovara(pitanje_objekt, )
        if not odgovor:
            # igrac je krivo odgovorio
            print("Točan odgovor je: " + pitanje_objekt.tocan_odgovor)
    ...
    def igrac_koristi_jockera(self, jocker, pitanje_objekt, redni_broj):
        if jocker.jocker == "pitaj_publiku":
            # print("tu: ", pitanje_objekt.odgovori)
            jockerov_odgovor = jocker.pitaj_publiku(pitanje_objekt.odgovori)
            print(jockerov_odgovor)
            self.prikaz.prikazi_pitanje(pitanje_objekt, redni_broj, self.igrac.iznosi[redni_broj - 1])
        elif jocker.jocker == "zovi":
            jockerov_odgovor = jocker.zovi(pitanje_objekt.odgovori)

            if jockerov_odgovor == pitanje_objekt.odgovor_a:
                oznaka = "A"
            elif jockerov_odgovor == pitanje_objekt.odgovor_b:
                oznaka ="B"
            elif jockerov_odgovor == pitanje_objekt.odgovor_c:
                oznaka = "C"
            else:
                oznaka = "D"
            ...
            jockerov_odgovor = jocker.pola_pola(pitanje_objekt.tocan_odgovor)
            pitanje_objekt.izbrisi_pitanja(jockerov_odgovor)
            self.prikaz.prikazi_pitanje(pitanje_objekt, redni_broj, self.igrac.iznosi[redni_broj - 1], jockerov_odgovor)
